...
import { AddRoutineTaskSchema } from "../../types";

export function AddRoutineTaskDrawer({
  routineId,
  children,
}: PropsWithChildren & { routineId: string }) {
...
function AddRoutineTaskForm({
  ...
  const form = useForm({
    resolver: zodResolver(AddRoutineTaskSchema),
    defaultValues: {
      name: "",
      competed: false,
    },
  ...
  return (
    <form
      onSubmit={form.handleSubmit((data) => {
        ...
        routineList.addRoutineTask({ routineId: routineId, task: data });
        onSuccess?.();
        form.reset();
      })}
